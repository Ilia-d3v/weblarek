...
export class CardPreview extends Component<IProduct> {
  ...
  set image(v: string) { const src = /^https?:\/\//.test(v) ? v : `${CDN_URL}/${v}`;
  this.setImage(this.imageEl, src, this.titleEl.textContent ?? ''); }
  set category(v: string) {
    this.categoryEl.textContent = v;
    this.categoryEl.className = `card__category ${categoryMap[v as keyof typeof categoryMap]}`;
  }
  set description(v: string) { this.textEl.textContent = v; }
